...
int buscarProdAlmacenClien(struct Producto almacenamiento[CAPACIDADMAX], struct Producto *ProductoAQuitar , int cantidadProducto)
{
    ...
    for (i=0; i < CAPACIDADMAX; i++)
    {
        if(almacenamiento[i].idProducto == ProductoAQuitar->idProducto && almacenamiento[i].stock >= cantidadProducto)
        {
            ...
            {
                printf("ALERTA: El producto con ID: %d tiene un stock menor a 10 unidades. Por Favor reponer productos lo antes posible. (Stock Disponible: %d)", almacenamiento[i].idProducto, almacenamiento[i].stock);
            }
            return 1;
        }
        else if (almacenamiento[i].idProducto == ProductoAQuitar->idProducto && almacenamiento[i].stock < cantidadProducto)
        {
            printf("ALERTA: El producto con ID: %d no tiene suficiente stock. Por Favor reponer productos lo antes posible. (Stock Disponible: %d)", almacenamiento[i].idProducto, almacenamiento[i].stock);
            return 0;
        }
    }
    return 0;
...
/*La función "buscarProdEnAlmacen" recorre el almacenamiento de una bodega en busca de un producto por su ID.
Si encuentra una coincidencia, crea un nuevo producto y devuelve un puntero a él con los detalles del producto.
...
struct Producto *buscarProdEnAlmacen(struct Producto almacenamiento[CAPACIDADMAX] , int idProd)
{
    ...
    for (i=0; i < CAPACIDADMAX ; i++)
    {
        if (almacenamiento[i].idProducto == idProd)
        {
            struct Producto *ProductoEncontrado = (struct Producto *)malloc(sizeof(struct Producto));
            ProductoEncontrado->idProducto = almacenamiento[i].idProducto;
            ProductoEncontrado->stock = almacenamiento[i].stock;
            ProductoEncontrado->nombre= (char *)malloc((strlen(almacenamiento[i].nombre) + 1) * sizeof(char));
            strcpy(ProductoEncontrado->nombre,almacenamiento[i].nombre);
            ProductoEncontrado->precio = almacenamiento[i].precio;
    ...
    while (transacciones != NULL)
    {
        if (transacciones->detalle != NULL && transacciones->detalle->productoComprado != NULL)
        {
            if (transacciones->detalle->idTransaccion == idTransaccion)
            {
                ...
                printf ("Nombre producto: %s\n", transacciones->detalle->productoComprado->nombre);
                printf ("Tipo de transaccion %s\n", transacciones->detalle->tipo);
                printf ("Cantidad Vendida/Comprada: %d\n", transacciones->detalle->cantidad);
                (*validacion) =1;
                return;
            }
        }
...
int main()
{
    ...
    while (opcion != 13)
    {
        ...
        else if (opcion == 10)
        {
            ...
            {
                printf("No se ha podido propoporcionar detalles de transaccion con el ID ingresado\n");
